...
import {AutomateModuleHelper} from "../../../contracts/integrations/AutomateModuleHelper.sol";
...
import {IGelato1Balance} from "../../../contracts/interfaces/IGelato1Balance.sol";
...
contract VRFGasReport is AutomateModuleHelper, Contracts {
    ...
    function setUp() public override {
        ...
        _dedicatedMsgSender = opsProxyFactory.determineProxyAddress(_users.user);
    ...
    function testExec1Balance() public {
        _createTaskVRF();
        bytes memory fulfillRandomnessData = _requestRandomnessAndGenerateFulfillRandomnessData();
        ...
        bytes memory execData = abi.encodeWithSelector(IOpsProxy.batchExecuteCall.selector, tos, datas, values);

        vm.prank(_users.gelato);
        automate.exec1Balance(_users.user, _dedicatedMsgSender, execData, _vrfModuleData(), _mock1BalanceParam(), true);
    }
    ...
    function testExec1BalanceSimple() public {
        ...
        bytes memory execData = abi.encodeWithSelector(
            IOpsProxy.executeCall.selector, address(_gelatoVRFConsumer), fulfillRandomnessData, 0
        );

        vm.prank(_users.gelato);
        automate.exec1BalanceSimple(
            _users.user, _dedicatedMsgSender, taskId, _mockCorrelationId(), execData, true, false
        );
    }
    ...
    function _createTaskVRF() private returns (bytes32 taskId) {
        ...
        taskId = automate.createTask(
            _dedicatedMsgSender, abi.encodePacked(IOpsProxy.batchExecuteCall.selector), _vrfModuleData(), address(0)
        );
    }

    function _vrfModuleData() private view returns (LibDataTypes.ModuleData memory moduleData) {
        moduleData = LibDataTypes.ModuleData({modules: new LibDataTypes.Module[](3), args: new bytes[](3)});
        ...
        string memory consumerAddressStr = Strings.toHexString(address(_gelatoVRFConsumer));
        ...
        moduleData.args[2] = _eventTriggerModuleArg(address(_gelatoVRFConsumer), topics, blockConfirmations);
    }
        ...
        returns (bytes memory fulfillRandomnessData)
    {
        ...
        bytes memory extraData = abi.encode(requestId, block.timestamp, block.number);
        bytes memory data = abi.encode(requestId, extraData);
        bytes memory dataWithRound = abi.encode(_round(), data);

        fulfillRandomnessData =
            abi.encodeWithSelector(IGelatoVRFConsumer.fulfillRandomness.selector, randomness, dataWithRound);
    }
    ...
    function _mock1BalanceParam() private view returns (IGelato1Balance.Gelato1BalanceParam memory oneBalanceParam) {
