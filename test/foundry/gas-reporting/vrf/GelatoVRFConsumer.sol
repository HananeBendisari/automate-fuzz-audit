...
contract GelatoVRFConsumer is GelatoVRFConsumerBase {
    ...
    event RandomnessFulfilled(uint256 indexed nonce, uint256 indexed fulfilledCount, Request request);
    ...
    function requestRandomness(uint256 _count) external {
        for (uint256 i; i < _count; i++) {
            _requestRandomness(abi.encode(nonce, block.timestamp, block.number));
        }
        nonce += 1;
    }

    function _fulfillRandomness(uint256 _randomness, uint256, bytes memory _extraData) internal override {
        (uint256 nonce_, uint256 requestTime, uint256 requestBlock) =
            abi.decode(_extraData, (uint256, uint256, uint256));

        Request memory request = Request(requestTime, requestBlock, block.timestamp, block.number, _randomness);
